...
fun calculateAltitude(pressure: Double, referencePressure: Double, referenceAirTemperature: Double, referenceAltitude: Double): Double {
    ...
    println("calculateAltitude: pressure = $pressure, referencePressure = $referencePressure, referenceAirTemperature = $referenceAirTemperature, referenceAltitude = $referenceAltitude")

    return referenceAltitude + if (referenceAltitude < 10000) {
        // barometric formula for altitudes below 10 km
        ...
        // above 10 km, the temperature lapse rate is assumed to be constant
        ln(referencePressure / pressure) * (UNIVERSAL_GAS_CONSTANT * referenceAirTemperature) / (GRAVITY * EARTH_AIR_MOLAR_MASS)
    }
}